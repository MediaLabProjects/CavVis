...
static void zprKeyboard(unsigned char key, int x, int y);
...
int t_show;
int flag=0;
int hide=0; // Key 'h' = hide surface
int j_show; // Key 'j' = show or hide surface of cavities or filling atoms
int c_show; // Key 'c' = change colors of cavities
int getH_show(){ return(hide);   }
int getJ_show(){ return(j_show); }
int getC_show(){ return(c_show); }
...
static void zprKeyboard(unsigned char key, int x, int y){
    ...
    // Generic incrementer for debugging purposes only
    if (key == 't'){ t_show++; changed=true;};
    
    if (key == 'h' || key == 'H'){
        if (hide == 0) hide = 1; else hide=0;
        changed=true;
    }
    if (key == 'c' || key == 'C'){
        if (c_show == 0) c_show = 1; else c_show=0;
        changed=true;
    }
    
    if (key == 'j' || key == 'J'){
        if (j_show == 0) j_show = 1; else j_show=0;
        changed=true;
    }
    
    if( key == 'r' ){
        glRotatef(10.0f,-(_maxx + _minx)/2.0f,-(_maxy + _miny)/2.0f,-(_maxz + _minz)/2.0f);
        changed=true;
    }
    if (key == 'w'){
        glTranslatef(((_maxx + _minx)/2.0f)*1,((_maxy + _miny)/2.0f)*1,((_maxz + _minz)/2.0f)*1);
...
static void zprReshape(int w,int h){
    ...
    /* Center protein object in the scene (default for the main implementation) */
    glTranslatef(-(_maxx + _minx)/2.0f,-(_maxy + _miny)/2.0f,-(_maxz + _minz)/2.0f);
